...
actor AIService {
    ...
    private var activeTasks: [String: Task<MarineData, Error>] = [:]
    
    func sendToAI(pngImage: Data, taskId: String = UUID().uuidString) async throws -> MarineData {
        // Create a cancellable task for this analysis
        let analysisTask = Task<MarineData, Error> {
            try Task.checkCancellation()
            
            logEvent("Sending binary image data with taskId: \(taskId)...", OSLog.ai)
        ...
        activeTasks[taskId] = analysisTask
    ...
    func cancelAnalysis(taskId: String) {
